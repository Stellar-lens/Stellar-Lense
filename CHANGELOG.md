...
- **GDPR deletion-witness accumulator (test-only), soundness-fuzzed**: `contracts/ledgerlens-score/src/test_gdpr_accumulator.rs` implements a from-scratch, sound replacement for the accumulator scaffold removed below in #395 -- a Merkle tree over every ever-inserted `(wallet, asset_pair)` key, where deletion flips a leaf's status rather than removing it, and a deletion witness is a standard Merkle inclusion proof that a leaf currently holds the deleted status. Property-fuzzed (6,000+ generated insert/delete sequences) for: a deletion witness verifies iff the element is currently deleted and hasn't been re-inserted since, and a witness for one element never verifies for another. It is a standalone data structure only -- not wired into `clear_score`/`clear_score_history`, matching how the original scaffold was also never wired; that integration is left for a follow-up. Closes #410.
