...
- **Error discriminant CI check**: New `error-discriminants` CI job (`tools/check_error_discriminants.sh`) fails a PR if it renames, removes, or renumbers any existing `Error` enum discriminant in `errors.rs`, since those values are part of the deployed contract's ABI. Purely additive changes (new discriminants, new `pub const` aliases) are unaffected. Closes #436.
