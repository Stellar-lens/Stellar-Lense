...
The capacity planning dashboard (`Stellar Lense — Capacity Planning`) projects compute requirements from trade volume growth so operators can make proactive infrastructure scaling decisions.
...
**Adding compute resources**: Scale the `stellar-lense-scorer` replicas in `docker-compose.yml` (or your orchestrator) by incrementing the replica count. The Kafka partition-key assignment distributes load across replicas automatically.
...
The linear-regression recording rules are defined in `alert_rules.yml` under the `stellar_lense_capacity_planning` group:

| Recording Rule | Description |
|---|---|
| `stellar_lense:cpu_usage_trend_slope` | Per-second derivative of CPU usage (least-squares over 7 days) |
| `stellar_lense:capacity_days_to_80pct_cpu` | Projected days until average CPU hits 80% |
| `stellar_lense:cpu_projection_ci_halfwidth` | 95% CI half-width on the projection |
...
|---|---|---|---|
| `stellar_lense_cpu_usage_ratio` | Gauge | `component` | CPU usage ratio (0.0–1.0) |
| `stellar_lense_memory_usage_bytes` | Gauge | — | Process RSS in bytes |
| `stellar_lense_trades_per_second` | Gauge | `asset_pair` | Trade event ingestion rate |
...
The per-pair health dashboard (`Stellar Lense — Per-Asset-Pair Health`) surfaces detection quality issues at the asset-pair level so operators can identify which specific trading pairs are experiencing problems without digging through aggregate metrics.
...
Use the **Asset Pair** Grafana variable dropdown at the top of the dashboard to filter all panels to a specific pair. The variable queries `label_values(stellar_lense_risk_score_distribution_bucket, asset_pair)`.
...
| Metric | Type | Labels |
|---|---|---|
| `stellar_lense_score_duration_seconds` | Histogram | `asset_pair` |
| `stellar_lense_benford_computation_total` | Counter | `asset_pair`, `status` |
| `stellar_lense_risk_score_distribution` | Histogram | `asset_pair` |
