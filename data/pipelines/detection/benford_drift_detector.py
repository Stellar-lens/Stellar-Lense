...
class BenfordBaseline:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "per_window_baselines": {str(k): v for k, v in self.per_window_baselines.items()},
...
class BenfordDriftDetector:
    ...
    def __init__(
        ...
        self.z_threshold = (
            z_threshold if z_threshold is not None else config.BENFORD_DRIFT_Z_THRESHOLD
        )
        ...
        try:
            ...
            logger.warning(
                f"Failed to initialize Benford drift detector DB: {e}. Using in-memory only."
            )
            self.engine = None
    ...
    def _save_baseline_to_db(self, baseline: BenfordBaseline) -> None:
        ...
        try:
            with Session(self.engine) as session:
                ...
                stmt = select(BenfordDriftModel).where(
                    BenfordDriftModel.pair_id == baseline.pair_id
                )
    ...
    def check(
        ...
        if not (np.isfinite(current_chi_square) and np.isfinite(current_mad)):
            logger.warning(
                f"Pair {pair_id}: invalid current metrics (chi_sq={current_chi_square}, mad={current_mad})"
            )
        ...
        chi_sq_z = (
            abs(current_chi_square - baseline.chi_square_mean) / chi_sq_std
            if chi_sq_std > 0
            else 0.0
        )
