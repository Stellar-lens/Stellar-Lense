...
def test_validate_require_onchain_missing_contract_id_raises(monkeypatch):
    ...
    assert "STELLARLENSE_CONTRACT_ID is not set." in str(exc.value)
...
def test_validate_require_onchain_missing_submitter_secret_raises(monkeypatch):
    ...
    assert "STELLARLENSE_SUBMITTER_SECRET is not set." in str(exc.value)
...
def test_validate_reports_multiple_errors_in_one_exception(monkeypatch):
    ...
    assert "STELLARLENSE_CONTRACT_ID is not set." in msg
    assert "STELLARLENSE_SUBMITTER_SECRET is not set." in msg
