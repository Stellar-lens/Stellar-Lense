...
def test_wallets_scored_total_increments():
    ...
    before = _get_counter_value("stellar_lense_wallets_scored", {"asset_pair": "XLM/TEST", "result": "above_threshold"})
    wallets_scored_total.labels(asset_pair="XLM/TEST", result="above_threshold").inc()
    after = _get_counter_value("stellar_lense_wallets_scored", {"asset_pair": "XLM/TEST", "result": "above_threshold"})
...
def test_soroban_submissions_total_increments():
    ...
    before = _get_counter_value("stellar_lense_soroban_submissions", {"status": "submitted"})
    soroban_submissions_total.labels(status="submitted").inc()
    after = _get_counter_value("stellar_lense_soroban_submissions", {"status": "submitted"})
    assert after == before + 1
...
def test_metrics_endpoint_returns_200_with_content():
    ...
    assert b"stellar_lense_wallets_scored_total" in resp.content
