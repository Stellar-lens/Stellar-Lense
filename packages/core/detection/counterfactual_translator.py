...
# ---------------------------------------------------------------------------
# Lazily-built lookup table so that importing this module does not
# trigger a full feature-constraint reconciliation at import time (that
# validation is deferred until the first call to translate_counterfactual).
# ---------------------------------------------------------------------------
_TRANSLATIONS: dict[str, str] | None = None


def _build_translations() -> dict[str, str]:
    """Build (once) the complete feature-name → plain-English mapping."""
    translations: dict[str, str] = {}

    for window, label in _WINDOW_LABELS.items():
        translations[f"benford_chi_square_{window}"] = (
            f"Reduce the statistical divergence between your trade amounts' leading digits and Benford's "
            f"Law within {label} windows -- avoid manufacturing amounts that fit an unnaturally precise "
            f"digit distribution."
        )
        translations[f"benford_mad_{window}"] = (
            f"Make the digit distribution of your trade amounts within {label} windows more consistent "
            f"with Benford's Law -- avoid clustering trades at psychologically round amounts."
        )
        translations[f"benford_max_zscore_{window}"] = (
            f"Avoid concentrating trade amounts so heavily on a single leading digit within {label} "
            f"windows that it produces a statistically extreme deviation from Benford's Law."
        )
        translations[f"max_stratum_chi2_{window}"] = (
            f"Reduce the worst-case digit-distribution divergence within any single amount stratum in "
            f"{label} windows, rather than just the aggregate."
        )
        translations[f"max_stratum_MAD_{window}"] = (
            f"Make the digit distribution within every amount stratum in {label} windows consistent with "
            f"Benford's Law, not just the overall distribution."
        )
        translations[f"n_flagged_strata_{window}"] = (
            f"Reduce the number of distinct amount strata in {label} windows whose digit distribution is "
            f"flagged as anomalous."
        )
        translations[f"ks_stat_{window}"] = (
            f"Reduce the Kolmogorov-Smirnov divergence between your trade amounts' digit distribution and "
            f"Benford's Law within {label} windows."
        )
        translations[f"ks_pval_{window}"] = (
            f"Trade so your {label} digit distribution is statistically indistinguishable from Benford's "
            f"Law under a Kolmogorov-Smirnov test, rather than showing significant deviation."
        )
        translations[f"kuiper_stat_{window}"] = (
            f"Reduce the Kuiper-test divergence between your trade amounts' digit distribution and "
            f"Benford's Law within {label} windows."
        )
        translations[f"kuiper_pval_{window}"] = (
            f"Trade so your {label} digit distribution is statistically indistinguishable from Benford's "
            f"Law under a Kuiper test, rather than showing significant deviation."
        )
        translations[f"benford_combined_flag_{window}"] = (
            f"Avoid triggering multiple independent Benford stratification/goodness-of-fit tests "
            f"simultaneously within {label} windows."
        )
...
def _validate_translations(translations: dict[str, str]) -> None:
    """Validate that every mutable feature has a translation entry.

    Raises :exc:`RuntimeError` if any mutable feature is missing.  Called
    lazily on first use so import-time side effects are avoided.
    """
    from detection.counterfactual_constraints import get_mutable_features

    missing = set(get_mutable_features()) - set(translations)
    if missing:
        raise RuntimeError(
            f"counterfactual_translator is missing entries for mutable features: {sorted(missing)}"
        )
...
def translate_counterfactual(deltas: dict) -> list[str]:
    ...
    # Fast-path: return immediately if already built (common case, also
    # avoids redundant work when called concurrently).
    if _TRANSLATIONS is not None:
        return [_TRANSLATIONS[name] for name in deltas]

    # Build and validate before assigning the global so a validation
    # failure does not leave _TRANSLATIONS in a half-built state that
    # the fast-path would return from on subsequent calls.
    translations = _build_translations()
    _validate_translations(translations)
