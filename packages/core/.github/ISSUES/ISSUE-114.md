...
## Summary
Exchange partners deploying Stellar Lense as a white-label solution currently share a single namespace: risk scores, alert rules, and suppressions from one exchange are visible to all. Row-level namespace isolation in the database and API key-to-namespace binding enables multiple exchanges to share a single Stellar Lense deployment while keeping their data strictly isolated.
...
- [ ] Admin key with `namespace: *` can query all namespaces (for the Stellar Lense operator)
