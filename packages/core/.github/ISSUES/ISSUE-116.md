...
## Summary
Stellar Lense has no formal release process: versions are not tagged, changelogs are not generated, and Docker images are not versioned beyond `latest`. Adopting semantic versioning with automated releases via GitHub Actions — triggered by conventional commits — provides a repeatable, auditable release process.
...
- [ ] Tag releases as `v1.2.3` in Git; push Docker image as `stellar_lense/api:1.2.3` and `:latest`
...
- [ ] `stellar_lense --version` outputs the version matching `pyproject.toml`
