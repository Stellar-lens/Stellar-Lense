...
## Summary
Exchange partners wishing to contribute to the Stellar Lense federated learning network currently must run the full `federated/fl_client.py` module with manual configuration. This issue packages the federated client as a standalone Python library (`stellar-lense-fl-client`) with a clean API, Docker image, and documented integration guide, lowering the barrier for exchange partners to join the federation without exposing their raw trade data.
...
Currently, the client is tightly coupled to the Stellar Lense monorepo internals, making it impractical for external exchanges to integrate. A standalone library with a stable, versioned API removes this friction.

## Objectives
- [ ] Extract `federated/fl_client.py` into a standalone `packages/stellar-lense-fl-client/` Python package
...
- [ ] Publish Docker image `stellar_lense/fl-client:latest` with configurable server URL and API key
...
The library must have zero hard dependencies on the Stellar Lense core monorepo.

## Definition of Done
- [ ] `pip install stellar-lense-fl-client` installs successfully in a clean virtualenv
