...
- **Latency spikes**: if the time from ledger close to score publish exceeds 60 seconds, the "real-time" claim of Stellar Lense is compromised
...
- The `/metrics` endpoint must be protected if `STELLARLENSE_ADMIN_API_KEY` is set — metrics can reveal operational intelligence (queue depths, error rates) that should not be publicly accessible.
...
- Add a startup check that warns if `METRICS_ENABLED=True` but `STELLARLENSE_ADMIN_API_KEY` is unset, as this exposes metrics publicly.
...
- Integration tests: start FastAPI test client; call `GET /metrics`; assert response is valid Prometheus text format containing `stellar_lense_ingestion_events_received_total`
...
- Create `docs/metrics.md` documenting all metric names, labels, types, and what alert thresholds are recommended (e.g., `stellar_lense_ingestion_events_dropped_total > 100/min` → investigate queue depth)
