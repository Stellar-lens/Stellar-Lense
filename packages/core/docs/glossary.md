...
Core domain and project-specific terms used throughout the Stellar Lense codebase and documentation.
...
Stellar Lense, the Benford engine (`detection/benford_engine.py`) applies chi-square, per-digit
...
gradient updates. Stellar Lense uses the **Krum** aggregation algorithm to achieve this. See
...
training distribution over time. In Stellar Lense this happens when wash-trading bots adapt their
...
book operated by a centralised entity. Stellar Lense targets the **Stellar DEX (SDEX)**, where
...
aggregation server. Stellar Lense uses federated learning to allow multiple exchange deployments
...
A class of neural network that operates directly on graph-structured data. In Stellar Lense, a GNN
...
sequential data. Stellar Lense uses an LSTM (or Transformer) encoder to process a wallet's ordered
...
A composite Benford divergence metric. In Stellar Lense, MAD is computed as the mean of the
...
signals significant drift that warrants model retraining. Stellar Lense declares drift when at
least three features exceed the PSI threshold. See [docs/drift_monitor.md](drift_monitor.md).

## RiskScore

The primary output schema of the Stellar Lense detection engine, defined in
...
fields. It is the shared contract between `stellar-lense-core`, `stellar-lense-api`, and the Soroban
on-chain registry.
...
activity. Stellar Lense risk scores and flagged wallet activity are designed to serve as supporting
...
In the Stellar Lense trade graph, an SCC with three or more accounts indicates a potential
...
via the Horizon API. Stellar Lense focuses specifically on wash-trading detection within the SDEX.
...
(wash-trading wallets) by interpolating between existing minority samples. Stellar Lense applies
SMOTE during model training to address the class imbalance inherent in fraud detection datasets.

## Soroban

Stellar's smart contract platform, built on WebAssembly (WASM) and written in Rust. Stellar Lense
deploys a `stellar-lense-score` Soroban contract that stores risk scores on-chain, making them
...
detection target of Stellar Lense.
...
Stellar Lense supports a **Groth16** ZK-SNARK backend for privacy-preserving threshold proofs,
