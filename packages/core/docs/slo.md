# Stellar Lense SLO / Error-Budget Framework

This document outlines the Service Level Objectives (SLOs), Service Level Indicators (SLIs), alerting design, and error-budget policies for Stellar Lense's key user journeys.

---

## Service Level Objectives (SLOs)

Stellar Lense defines four core SLOs over a rolling **30-day window**:
...
| **Soroban Submission** | % of on-chain submissions resulting in `status="success"` (excluding dry runs) | **99.0%** | Guarantees auditability of Stellar Lense classifications on the Soroban smart contract network. |

---

## Alerting Design (Multi-Window, Multi-Burn-Rate)

Instead of relying solely on single-threshold alerts (which cause alert fatigue or miss slow degradations), Stellar Lense implements **multi-window, multi-burn-rate alerting** as described in the Google SRE workbook.
...
Operators can query `GET /admin/slo-status` (gated by `X-StellarLense-Admin-Key`) to retrieve the current error budget consumption per SLO:

```bash
curl -H "X-StellarLense-Admin-Key: <key>" http://localhost:8000/v1/admin/slo-status
```
