...
This runbook outlines the lifecycle, configuration, and step-by-step procedures for rotating the four secret types managed in Stellar Lense:
1. **Scoped API Keys** (`detection/api_key_store.py`)
2. **Namespace API Keys** (`api/namespace.py`)
3. **Webhook HMAC Encryption Key** (`STELLARLENSE_WEBHOOK_ENCRYPTION_KEY`)
4. **Soroban Service Account Key** (`STELLARLENSE_SERVICE_SECRET_KEY`)
...
```bash
curl -X POST "https://<stellar-lense-host>/admin/api-keys/<key_id>/rotate?grace_period_seconds=604800" \
     -H "X-StellarLense-Admin-Key: <admin_key>"
```
...
```bash
curl -X POST "https://<stellar-lense-host>/admin/namespaces/<namespace_id>/rotate-key?grace_period_seconds=604800" \
     -H "X-StellarLense-Admin-Key: <admin_key>"
```
...
1. Set the current key as the previous key and generate a new key as current:
   - Current Key: `STELLARLENSE_WEBHOOK_ENCRYPTION_KEY`
   - Previous Key: `STELLARLENSE_WEBHOOK_ENCRYPTION_KEY_PREVIOUS`
...
5. Remove `STELLARLENSE_WEBHOOK_ENCRYPTION_KEY_PREVIOUS` from the environment and redeploy.

---

## 4. Soroban Service Account Key
The Soroban service account key is an on-chain authorized Stellar account key. Full automation is not possible due to contract-level authorization ownership residing in `stellar-lense-contracts`.
...
[Dual-Signature Window: Both Old and New Keys Authorized]
     |
     +--> Deploy New Key to Stellar Lense
...
2. **On-chain Authorization**: Submit a transaction to authorize the new public key to call `submit_score()` on-chain via the appropriate `stellar-lense-contracts` script.
3. **Dual-Sign Window**: Ensure both the old and new keys are authorized simultaneously.
4. **Deploy & Test**: Set the new secret key as `STELLARLENSE_SERVICE_SECRET_KEY` in the environment, restart the process, and submit a test score transaction on Testnet.
5. **Retire Old Key**: Once verified, submit an on-chain transaction to revoke the old key's authorization.

---

## Operator Checklist
- [ ] Monitor `stellar_lense_secret_rotation_total` for success and failure counts.
- [ ] Respond immediately to `SecretRotationOverdue` alert.
- [ ] Limit the period when both `STELLARLENSE_WEBHOOK_ENCRYPTION_KEY` and `STELLARLENSE_WEBHOOK_ENCRYPTION_KEY_PREVIOUS` are loaded.
