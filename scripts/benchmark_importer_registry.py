...
def print_results(results: list[BenchmarkResult]) -> None:
    ...
    for result in results:
        ...
        print(f"{result.name:<45} {iterations_str:<12} {avg_time_str:<15} {ops_str:<15} {memory_str:<10}")
...
def print_analysis(results: list[BenchmarkResult]) -> None:
    ...
    if query_results:
        ...
        print(f"Expected query latency: {'< 1ms (excellent)' if avg_query_time < 1 else '< 10ms (good)' if avg_query_time < 10 else 'needs optimization'}")
    
    # Memory analysis
    memory_result = next((r for r in results if r.memory_mb), None)
    if memory_result:
        print(f"\nMemory per importer: ~{memory_result.memory_mb / 1000:.4f} MB")
        print(f"Memory efficiency: {'excellent' if memory_result.memory_mb < 10 else 'acceptable' if memory_result.memory_mb < 50 else 'needs optimization'}")
