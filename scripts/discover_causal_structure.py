...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dag-output", default="models/causal_dag.json", help="Path to save the JSON DAG structure"
    )
    parser.add_argument(
        "--img-output",
        default="reports/causal_dag.png",
        help="Path to save the DAG visualization image",
    )
    parser.add_argument(
        "--alpha", type=float, default=0.05, help="Significance level for independence tests"
    )
...
def main() -> None:
    ...
    if not os.path.exists(args.data_path):
        print(
            f"Data file '{args.data_path}' not found. Generating a synthetic dataset of 500 wallets..."
        )
