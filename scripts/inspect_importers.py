    ...
    if args.capability:
        ...
        for cap_name in args.capability.split("|"):
            ...
            try:
                ...
                print(f"Available: {', '.join(c.name for c in ImporterCapability if c != ImporterCapability.NONE)}")
                return 1
...
def cmd_report(args: argparse.Namespace) -> int:
    ...
    for name in importers:
        ...
        if info.performance.typical_latency_ms or info.performance.throughput_records_per_sec:
            ...
            if info.performance.throughput_records_per_sec:
                lines.append(f"- Throughput: ~{info.performance.throughput_records_per_sec} records/sec")
            lines.append("")
...
def cmd_json(args: argparse.Namespace) -> int:
    ...
    for name in importers:
        ...
        data["importers"][name] = {
            ...
            "capabilities": [cap.name for cap in ImporterCapability if cap != ImporterCapability.NONE and info.has_capability(cap)],
