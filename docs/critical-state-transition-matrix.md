...
`ledgerlens-aggregator` contracts at the time of issues #779, #781, and #782.
...
- **Resources:** one bounded host `require_auth` operation was added to each
  one-time initializer. No loop, collection, or persistent entry was added.
  Existing worst-case bounds remain `MAX_BATCH_SIZE`, `BATCH_READ_MAX`,
  bounded per-pair history, and `MAX_SHARDS = 10`.
