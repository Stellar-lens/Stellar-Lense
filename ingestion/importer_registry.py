...
class ImporterRegistry:
    ...
    def unregister(self, name: str) -> None:
        ...
        metadata = self._importers.pop(name)
    ...
    def get_importer_info(self, name: str) -> ImporterMetadata:
        ...
        if name not in self._importers:
            ...
            raise KeyError(
                f"Importer {name!r} not found in registry. "
    ...
    def find_best_match(
        ...
        if required_capabilities:
            candidates = {
                name: meta for name, meta in candidates.items()
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "importers_by_data_type": {
                str(dt): len(importers)
                for dt, importers in self._by_data_type.items()
            },
            "importers_by_source": {
                str(src): len(importers)
                for src, importers in self._by_source.items()
            },
            "streaming_importers": len(
                self._by_capability.get(ImporterCapability.STREAMING, set())
            ),
            "bulk_importers": len(
                self._by_capability.get(ImporterCapability.BULK, set())
            ),
