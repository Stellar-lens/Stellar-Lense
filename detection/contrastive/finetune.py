...
def finetune(
    ...
    if os.path.exists(pretrained_encoder_path):
        ...
        logger.warning(
            f"Pre-trained encoder not found at {pretrained_encoder_path}. Using random weights!"
        )
    ...
    for epoch in range(epochs):
        ...
        logger.info(
            f"Epoch [{epoch+1}/{epochs}] Loss: {total_loss/len(train_loader):.4f} | Val Acc: {val_acc:.4f}"
        )
...
if __name__ == "__main__":
    ...
    finetune(
        "./models/pretrained_encoder.pt", train_feats, train_labels, val_feats, val_labels, epochs=5
    )
