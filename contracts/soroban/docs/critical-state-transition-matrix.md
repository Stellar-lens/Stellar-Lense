...
This matrix records the concrete behavior of the `stellar-lense-score` and
`stellar-lense-aggregator` contracts at the time of issues #779, #780, #781, and
#782.
...
| Batch submission | Same as single submission; `1..=MAX_BATCH_SIZE` entries | Returns one indexed result per input and processes within the fixed cap | Empty and over-limit batches reject; per-entry failures are explicit and cannot silently become successes | `test_batch_*`, `test_batch_attestation::*`; batch resource benches in `contracts/stellar-lense-score/benches` |
