...
//! canonical [`stellar_lense_score::RiskScore`] contract type.
//!
...
//!
//! * natively via [`stellar_lense_score::RiskScore::spec_xdr()`] — the exact
...
//! [`stellar_lense_score::constants`] range constants so the emitted schema
...
pub fn native_risk_score_struct() -> Result<ScSpecUdtStructV0> {
    let bytes = stellar_lense_score::RiskScore::spec_xdr();
...
pub fn risk_domain_range(field: &str) -> Option<(u32, u32)> {
    match field {
        "score" | "confidence" | "benford_score" | "ml_score" | "network_score" => {
            Some((stellar_lense_score::constants::MIN_SCORE, stellar_lense_score::constants::MAX_SCORE))
        }
        _ => None,
    }
}
...
pub fn struct_to_json_schema(struct_: &ScSpecUdtStructV0) -> Value {
    ...
    let mut root = json!({
        ...
        "$id": "https://stellar_lense.dev/schemas/risk_score.schema.json",
...
pub fn struct_to_typescript(struct_: &ScSpecUdtStructV0) -> String {
    let mut lines = vec![
        ...
        "// contracts/stellar_lense-score/src/types.rs (`RiskScore`). Do not edit by hand."
            .to_string(),
...
pub fn struct_to_python(struct_: &ScSpecUdtStructV0) -> String {
    ...
    let mut lines = vec![
        "# Generated by tools/schema-gen from the Soroban contract-spec of".to_string(),
        "# contracts/stellar_lense-score/src/types.rs (`RiskScore`). Do not edit by hand.".to_string(),
        "# Fields are in contract-spec (XDR) order — alphabetical, matching the on-chain spec."
            .to_string(),
...
mod tests {
    ...
    fn score_range_is_driven_by_contract_constants_not_a_copy() {
        ...
        assert_eq!(
            ...
            Some(u64::from(stellar_lense_score::constants::MIN_SCORE)),
            "`score.minimum` must equal the contract's MIN_SCORE constant"
        );
        assert_eq!(
            score["maximum"].as_u64(),
            Some(u64::from(stellar_lense_score::constants::MAX_SCORE)),
