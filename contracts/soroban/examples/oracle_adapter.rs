//! Reference integration: registering a price-feed oracle with StellarLense and
...
//! 2. Registering it with StellarLense via `register_oracle`.
...
//!
//! ```text
//! cargo build --example oracle_adapter -p stellar_lense-score
//! ```
//!
...
/// sufficient to demonstrate the StellarLense integration.
///
/// **The only requirement** StellarLense places on a registered oracle is that
...
impl ExamplePriceFeedOracle {
    ...
    ///
    /// This is the function StellarLense calls via cross-contract invocation
...
//
// 1. Deploy StellarLense and the oracle:
//
//    let ll_id  = env.register_contract(None, StellarLenseScoreContract);
//    let orc_id = env.register_contract(None, ExamplePriceFeedOracle);
//    let ll  = StellarLenseScoreContractClient::new(&env, &ll_id);
...
//
// 3. Register the oracle with StellarLense (admin call):
//
