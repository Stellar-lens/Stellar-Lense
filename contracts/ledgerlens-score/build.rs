...
fn main() {
    ...
    let errors_content = fs::read_to_string(&errors_path).expect("Failed to read errors.rs");
    ...
    let tests_content = fs::read_to_string(&tests_path).expect("Failed to read error_coverage.rs");

    let test_count =
        tests_content.lines().filter(|line| line.trim().starts_with("fn test_error_")).count();
