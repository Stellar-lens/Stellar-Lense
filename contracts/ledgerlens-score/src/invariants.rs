...
pub fn invariant_check(env: &Env) {
    ...
    assert!(min_conf <= 100, "INVARIANT VIOLATED: global_min_confidence={min_conf} exceeds 100");
    ...
    assert!(denom > 0, "INVARIANT VIOLATED: decay_rate_denominator=0 (division by zero risk)");

    // 5. Gate query fee must be non-negative.
    let gate_fee = storage::get_gate_query_fee(env);
    assert!(gate_fee >= 0, "INVARIANT VIOLATED: gate_query_fee={gate_fee} is negative");

    // 6. Accumulated fees must be non-negative.
    let accum = storage::get_accumulated_fees(env);
    assert!(accum >= 0, "INVARIANT VIOLATED: accumulated_fees={accum} is negative");
}
