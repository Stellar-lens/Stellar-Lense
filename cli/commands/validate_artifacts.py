...
def validate_artifacts(artifacts_dir: str = "artifacts") -> Dict[str, Any]:
    if not os.path.exists(artifacts_dir):
        return {
            "status": "FAIL",
            "error": f"Artifacts directory '{artifacts_dir}' does not exist."
        }
    ...
    if not os.path.exists(metadata_path):
        return {
            "status": "FAIL",
            "error": f"Required artifact file missing: {metadata_path}"
        }
    ...
    try:
        ...
        if missing_keys:
            return {
                "status": "FAIL",
                "error": f"Metadata missing required fields: {missing_keys}"
            }
